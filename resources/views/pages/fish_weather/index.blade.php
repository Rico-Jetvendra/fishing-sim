@include('components.header', ['title' => 'Fish Weather'])
...
<main class="app-main">
    <div class="app-content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h3 class="mb-0">Fish Weather</h3>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-end">
                        <li class="breadcrumb-item"><a href="{{ route('web.index') }}">Dashboard</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Fish Weather</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <form method="POST" id="crudForm" onsubmit="store()">
                ...
                <div class="modal-header">
                    <h5 class="modal-title">
                        <span id="modalTitle">Add</span> Fish Weather
                    </h5>
            ...
            <form id="relationForm" onsubmit="saveRelation()">
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-12 col-md-12 mb-3">
                            <label class="form-label" for="weather_id">Weather</label>
                            <select class="form-select searchable-select" name="weather_id" id="weather_id" required>
                                <option value="">-- Select Weather --</option>
                                @foreach($selects['weather'] as $weather)
                                    <option value="{{ $weather->weather_id }}">
                        ...
                        <div class="col-lg-12 col-md-12 mb-3">
                            <label class="form-label" for="weather_modifier">Modifier</label>
                            <input type="number" class="form-control" name="weather_modifier" step="1" min="0" max="99999" id="weather_modifier" placeholder="Weather Modifier" value="0" oninput="if(this.value > 99999) this.value = 99999; if(this.value < 1) this.value = 0;"/>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    initCrud({
        routes: {
            store: "{{ route('web.fishWeather.store') }}",
            update: id => "{{ route('web.fishWeather.update', ':id') }}".replace(':id', id),
            edit: id => "{{ route('web.fishWeather.edit', ':id') }}".replace(':id', id),
            destroy: id => "{{ route('web.fishWeather.destroy', ':id') }}".replace(':id', id),
            data: "{{ route('web.fishWeather.data') }}"
        },
        fields: {
            'fish_id': 'fish_name',
    ...
    function populateRelation(){
        ...
        relations.forEach((relation, index) => {
            tbody.append(`
                <tr>
                    <td>${index+1}</td>
                    <td>${relation.weather_id['name']}</td>
                    <td>${relation.weather_modifier}</td>
                    <td>
    ...
    function editRelation(index){
        ...
        form.find('#weather_id')[0].tomselect.setValue(item.weather_id['id']);
        form.find('#weather_modifier').val(item.weather_modifier);
    ...
    function store(){
        ...
        if($('#formMethod').val() == 'POST'){
            $.post("{{ route('web.fishWeather.store') }}", data).then((result) => {
                if(!result.success){
                    Swal.fire({
                        title: 'Error',
            ...
            $.ajax({
                url: "{{ route('web.fishWeather.update', ':id') }}".replace(':id', id),
                type: 'POST',
                data: data,
                dataType: 'json',
    ...
    $('.btn-create').click(() => {
        ...
        form.attr('action', "{{ route('web.fishWeather.store') }}");
        $('#formMethod').val('POST');
        $('#modalTitle').text('Add');
    ...
    $(document).on('click', '.btn-edit-relation', function () {
        ...
        $.get("{{ route('web.fishWeather.edit', ':id') }}".replace(':id', id))
        .done(function(res){
            relations.length = 0;
            ...
            res.forEach(element => {
                let data = {
                    weather_id: {
                        id: element.weather_id,
                        name: element.weather_name
                    },
                    weather_modifier: element.weather_modifier,
                };
    ...
    function destroy(id) {
        $.post("{{ route('web.fishWeather.destroy', ':id') }}".replace(':id', id), {
            _method: 'DELETE'
        })
        .done(() => {
            Swal.fire({
